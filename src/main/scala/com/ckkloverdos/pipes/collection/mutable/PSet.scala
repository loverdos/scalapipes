...
object PSet {
  ...
  @inline final def ofIterable[A]: Iterable[A] ⇒ Set[A] = _.to[Set]

  @inline final def ofIterator[A]: Iterator[A] ⇒ Set[A] = _.to[Set]

  @inline final def ofSeq[A]: Seq[A] ⇒ Set[A] = _.to[Set]

  @inline final def ofList[A]: List[A] ⇒ Set[A] = _.to[Set]

  @inline final def ofArray[A]: Array[A] ⇒ Set[A] = _.to[Set]

  @inline final def ofMap[A, B]: Map[A, B] ⇒ Set[(A, B)] = _.to[Set]
